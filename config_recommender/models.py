...
class ModelArchitecture:
    ...
    _kv_cache_detail: Optional[KVCacheDetail] = field(
    ...
    def get_model_memory_gb(self) -> float:
        ...
        if (
            ...
            and self.num_parameters is None
        ):
    ...
    def get_kv_cache_gb(self, context_len: int, batch_size: int = 1) -> float:
        ...
        if (
            ...
            return kv_cache_req(
                self._model_info, self._model_config, context_len, batch_size
            )
        ...
        if (
            ...
            num_kv_heads = (
                self.num_kv_heads if self.num_kv_heads else self.num_attention_heads
            )
    ...
    def get_kv_cache_detail(
        self, context_len: int, batch_size: int = 1
    ) -> Optional[KVCacheDetail]:
        ...
        if (
            self._kv_cache_detail is None
            or self._kv_cache_detail.context_len != context_len
            or self._kv_cache_detail.batch_size != batch_size
        ):
